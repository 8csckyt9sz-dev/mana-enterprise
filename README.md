...
Cloudflare Workers + D1 で動く中古車・中古家具家電の在庫サイトです。公開ページは誰でも閲覧でき、`/admin/` と `/admin/api/` は Cloudflare Access とWorker内のJWT検証で保護します。画像もD1へ保存するためR2契約は不要です。
...
4. `npx wrangler secret put ADMIN_EMAIL`
5. `CF_ACCESS_TEAM_DOMAIN` と `CF_ACCESS_AUD` を本番値に設定（機密扱いする場合は両方とも `wrangler secret put` を使用し、`vars` から削除）
6. `pnpm db:migrate:remote`
7. `pnpm deploy`

Cloudflare Zero Trust の Access > Applications で Self-hosted application を作り、`https://<worker-domain>/admin/*` を保護します。Allow policy は管理者メール1件だけにします。管理APIも同じパス配下なので保護されます。
